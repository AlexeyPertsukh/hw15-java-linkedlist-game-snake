...
public class Map {
    ...
    private String getColor(int val) {
        switch (val){
            ...
            case Const.APPLE:
                return Const.COLOR_APPLE;   //цвет змеи
            case Const.STONE:
                return Const.COLOR_STONE;   //цвет змеи
            default:
    ...
    //копируем местиоположение змейки в игровое поле
        private void fromPointsToArr(Point firstPoint) {    //
        int x;
        int y;
    ...
    public void action (Snake snake){
        clear();
        fromPointsToArr(snake.getFirstPoint());
...
//        System.out.println(getFreeCell()+" >>"+isFreeCell10proc());
